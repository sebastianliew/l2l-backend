...
export class ItemSalesController {
  static async getItemSalesReport(
    ...
    try {
      ...
      // Lookup product details to get cost price
      pipeline.push({
        $lookup: {
          from: 'products',
          let: { 
              ...
              $cond: {
                if: {
                  $and: [
                    ...
                    { $eq: [{ $strLenCP: '$items.productId' }, 24] },
                    { $regexMatch: { input: '$items.productId', regex: '^[0-9a-fA-F]{24}$' } }
                  ]
                },
      ...
      // Add product cost info to items
      pipeline.push({
        $addFields: {
          'items.costPrice': {
            $ifNull: [
              { $arrayElemAt: ['$productInfo.costPrice', 0] },
              0
            ]
          }
        }
      ...
      pipeline.push({
        $group: {
          ...
          average_cost_price: { $avg: { $ifNull: ['$items.costPrice', 0] } },
