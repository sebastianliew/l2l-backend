...
interface RevenueAnalysisOptions {
  period?: string;
  startDate?: Date;
  endDate?: Date;
}
...
export class RevenueAnalysisService {
  static async getRevenueAnalysis(options: RevenueAnalysisOptions | string = '6months'): Promise<RevenueAnalysisResponse> {
    ...
    if (typeof options === 'string') {
      const months = options === '6months' ? 6 : 12
      startDate = new Date()
      ...
      if (options.startDate && options.endDate) {
        ...
        const months = options.period === '6months' ? 6 : 12
        startDate = new Date()
        startDate.setMonth(startDate.getMonth() - months)
        startDate.setDate(1) // Start from beginning of month
        endDate = new Date()
